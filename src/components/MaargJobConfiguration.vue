...
export default defineComponent({
  ...
  methods: {
        ...
        .create({
          ...
          buttons: [
            ...
            },
            {
              ...
              handler: async () => {
                try {
                  const resp = await JobService.runNow(this.currentMaargJob.jobName)
      ...
      const alert = await alertController
        .create({
          ...
          buttons: [{
            ...
            handler: async () => {
              try {
                const resp = await JobService.updateMaargJob({ ...job, paused: "Y" })
                if(!hasError(resp)) {
                  showToast(translate("Job has been cancelled succesfully."))
                  this.store.dispatch("job/updateMaargJob", job.jobName)
                } else {
                  throw resp.data
                }
              } catch(err) {
    ...
    async updateJob() {
      ...
      try {
        const resp = await JobService.updateMaargJob(payload)
        if(!hasError(resp)) {
